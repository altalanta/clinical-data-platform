...
dbt.docs: ## Generate dbt documentation and copy to docs site
	@echo "$(BLUE)Generating dbt documentation...$(NC)"
	poetry run dbt docs generate --project-dir dbt_project/ --target local
	@mkdir -p docs/assets/dbt/
	@cp -r dbt_project/target/* docs/assets/dbt/
	@echo "$(GREEN)dbt documentation copied to docs/assets/dbt/$(NC)"

eval: ## Run model evaluation with cross-validation and calibration
	...
	@echo "$(GREEN)Model evaluation completed! Check artifacts/eval/$(NC)"
